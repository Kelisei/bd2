...
class ToursApplicationTests {
	...
	void createAndGetUserTest() throws ToursException {
		User user1 = this.toursService.createUser("user1", "1234", "Usuario Uno", "user1@gmail.com", dob1,
				"000111222333");
		...
		DriverUser driverUser1 = this.toursService.createDriverUser("userD", "1234", "Usuario Driver",
				"userd@gmail.com", dob2, "000111222444", "exp...");
		assertNotNull(driverUser1.getId());
		TourGuideUser tourGuideUser1 = this.toursService.createTourGuideUser("userG", "1234", "Usuario TourGuide",
				"userg@gmail.com", dob2, "000111222555", "edu...");
		...
		assertThrows(ToursException.class, () -> this.toursService.createUser("userD", "1234", "Otro usuario",
				"otromail@gmail.com", dob1, "000111222999"), "Constraint Violation");
	}
	...
	void updateUserTest() throws ToursException {
		User user1 = this.toursService.createUser("user1", "1234", "Usuario Uno", "user1@gmail.com", dob1,
				"000111222333");
		DriverUser driverUser = this.toursService.createDriverUser("userD", "1234", "Usuario Driver", "userd@gmail.com",
				dob2, "000111222444", "exp...");
	...
	void assignWorkersToRoutesTest() throws ToursException {
		...
		List<Stop> stops1 = new ArrayList<Stop>(Arrays.asList(stop1, stop2, stop3));
		Route route1 = this.toursService.createRoute("Estadios", 20000, 55.5f, 3, stops1);
		DriverUser driverUser1 = this.toursService.createDriverUser("userD1", "1234", "Usuario Driver",
				"userd1@gmail.com", dob2, "000111222444", "exp...");
		DriverUser driverUser2 = this.toursService.createDriverUser("userD2", "1234", "Usuario Driver",
				"userd2@gmail.com", dob2, "000111222444", "exp...");
		TourGuideUser tourGuideUser1 = this.toursService.createTourGuideUser("userG1", "1234", "Usuario TourGuide",
				"userg1@gmail.com", dob2, "000111222555", "edu...");
		...
		assertThrows(ToursException.class,
				() -> this.toursService.assignTourGuideByUsername("user_no_existente", tourGuideUser1.getId()),
				"No pudo realizarse la asignación");
		assertThrows(ToursException.class,
				() -> this.toursService.assignDriverByUsername(driverUser1.getUsername(), 1000000L),
				"No pudo realizarse la asignación");
	}
	...
	void createAndGetSupplierAndService() throws ToursException {
		...
		Optional<Service> optionalService1 = this.toursService.getServiceByNameAndSupplierId("Servicio1",
				supplier1.getId());
		...
		Optional<Service> optionalService2 = this.toursService.getServiceByNameAndSupplierId("Servicio2",
				supplier1.getId());
		assertFalse(optionalService2.isPresent());

		assertThrows(ToursException.class, () -> this.toursService.createSupplier("Supplier2", "000111"),
	...
	void updateServicePriceTest() throws ToursException {
		...
		assertThrows(ToursException.class, () -> this.toursService.updateServicePriceById(100000L, 500f),
	...
	void createAndGetPurchaseTest() throws ToursException {
		...
		assertThrows(ToursException.class, () -> this.toursService.createPurchase("200", dyes, route1, user1),
				"No puede realizarse la compra");
		assertThrows(ToursException.class, () -> this.toursService.createPurchase("100", route1, user1),
				"Constraint Violation");
	}
	...
	void deleteUserTest() throws ToursException {
		...
		User user2 = this.toursService.createUser("user2", "1234", "Usuario Dos", "user2@gmail.com", dob2,
				"000111222334");
		Stop stop1 = this.toursService.createStop("Estadio Monumental", "Estadio de River Plate");
		Stop stop2 = this.toursService.createStop("Estadio La Bombonera", "Estadio de Boca Junions");
		Stop stop3 = this.toursService.createStop("Estadio Libertadores de America", "Estadio de Independiente");
		...
		assertThrows(ToursException.class, () -> this.toursService.deleteUser(user2b),
				"El usuario se encuentra desactivado");

		TourGuideUser tourGuideUser = this.toursService.createTourGuideUser("userG", "1234|", "Usuario TourGuide",
				"userg@gmail.com", dob2, "000111222555", "edu...");
		this.toursService.assignTourGuideByUsername(tourGuideUser.getUsername(), route1.getId());
		assertTrue(tourGuideUser.isActive());
		assertThrows(ToursException.class, () -> this.toursService.deleteUser(tourGuideUser),
				"El usuario no puede ser desactivado");
	}
